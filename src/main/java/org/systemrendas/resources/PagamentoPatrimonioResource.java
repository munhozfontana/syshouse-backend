...
public class PagamentoPatrimonioResource {
    ...
    public Response listAllPage(@QueryParam Integer page, @QueryParam Integer size) {
        Pagination<PagamentoPatrimonioDTO> pages = service.findAllPage(page, size);
    ...
    public Response insert(final @RequestBody @Valid PagamentoPatrimonioNewDTO dto) throws URISyntaxException {
        PagamentoPatrimonio entidade = null;
        ...
        entidade.setId(id);
        return Response.created(new URI("pagamentopatrimonio/" + id.toString())).entity(entidade).build();
    }

    @PUT
    @Path("{id}")
    @Operation(summary = "Atualiza PagamentoPatrimonio", description = "Atualiza o objeto PagamentoPatrimonio")
    public Response update(@PathParam("id") final UUID id, final @Valid PagamentoPatrimonioDTO dto) {
