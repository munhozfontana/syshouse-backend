...
public class MunicipioResourceTest {
    ...
    private final int elementsToInsert = 6;
    ...
    public void testFindById() {
        ...
        requisicao().pathParam("id", result[0].getId()).when().get(PATH + "/{id}").then()
                .statusCode(Response.Status.OK.getStatusCode()).body("nome", CoreMatchers.equalTo("nome"));
    }
    ...
    public void testUpdate() {
        ...
        body.put("nome", "nomeDiferente");
        requisicao().pathParam("id", result[0].getId()).body(body).when().put(PATH + "/{id}").then()
                .statusCode(Response.Status.OK.getStatusCode()).body("nome", CoreMatchers.equalTo("nomeDiferente"));
    }
