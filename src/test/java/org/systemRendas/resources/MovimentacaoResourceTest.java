...
public class MovimentacaoResourceTest {
    ...
    private static final String PATH_MOVIMENTACAO = "/movimentacao";
    ...
    private HashMap<String, Object> bodyMovimentacao = new HashMap<String, Object>();
    ...
    public void testInsert() {
        bodyMovimentacao.put("patrimonioIn", idPatrimonio);
        bodyMovimentacao.put("patrimonioOut", idPatrimonio);
        bodyMovimentacao.put("valor", 0);
        bodyMovimentacao.put("data ", "06/04/2004");
        bodyMovimentacao.put("obs", atibuteValue);

        requisicao().body(bodyMovimentacao).when().post(PATH_MOVIMENTACAO).then()
                .statusCode(Response.Status.CREATED.getStatusCode()).header("location", CoreMatchers.notNullValue());
    }
    ...
    public void testListAll() {
        idMovimentacao = requisicao().when().get(PATH_MOVIMENTACAO).then()
    ...
    public void testListAllPegeable() {
        final int page = 0;
        final int size = 5;
        requisicao().queryParam("page", page).queryParam("size", size).when().get(PATH_MOVIMENTACAO + "/page").then()
    ...
    public void testFindById() {
        requisicao().pathParam("id", idMovimentacao).when().get(PATH_MOVIMENTACAO + "/{id}").then()
    ...
    public void testUpdate() {
        Map<String, Object> body = bodyMovimentacao;
        body.put(atibuteValue, "novoDadoDiferente");
        requisicao().pathParam("id", idMovimentacao).body(body).when().put(PATH_MOVIMENTACAO + "/{id}").then()
    ...
    public void testDelete() {
        requisicao().pathParam("id", idMovimentacao).when().delete(PATH_MOVIMENTACAO + "/{id}").then()
                .statusCode(Response.Status.NO_CONTENT.getStatusCode());

        requisicao().when().get(PATH_MOVIMENTACAO).then().statusCode(Response.Status.OK.getStatusCode()).body("size()",
                CoreMatchers.is(0));
