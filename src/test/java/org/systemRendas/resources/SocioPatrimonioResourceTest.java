...
public class SocioPatrimonioResourceTest {
    ...
    public void testListAllPegeable() {
        final int page = 0;
        final int size = 5;
        requisicao().queryParam("page", page).queryParam("size", size).when().get(PATH_SOCIOPATRIMONIO + "/page").then()
    ...
    public void testFindById() {
        requisicao().pathParam("id", idSocioPatrimonio).when().get(PATH_SOCIOPATRIMONIO + "/{id}").then()
                .statusCode(Response.Status.OK.getStatusCode()).body("proprietario", CoreMatchers.equalTo(true));
    }
    ...
    public void testUpdate() {
        ...
        requisicao().pathParam("id", idSocioPatrimonio).body(body).when().put(PATH_SOCIOPATRIMONIO + "/{id}").then()
                .statusCode(Response.Status.OK.getStatusCode()).body("proprietario", CoreMatchers.equalTo(false));
    }
